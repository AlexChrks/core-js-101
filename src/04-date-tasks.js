...
function timeSpanToString(startDate, endDate) {
  ...
  const fullSeconds = (gap - milliseconds) / 1000;
  const seconds = fullSeconds % 60;
  const fullMinutes = (fullSeconds - seconds) / 60;
  const minutes = fullMinutes % 60;
  const hours = (fullMinutes - minutes) / 60;

  const outputHours = hours.toString().padStart(2, '0');
  const outputMinutes = minutes.toString().padStart(2, '0');
  const outputSeconds = seconds.toString().padStart(2, '0');
  const outputMilliseconds = milliseconds.toString().padStart(3, '0');
  return `${outputHours}:${outputMinutes}:${outputSeconds}.${outputMilliseconds}`;
}
